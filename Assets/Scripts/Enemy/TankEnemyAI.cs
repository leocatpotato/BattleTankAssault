...
public class TankEnemyAI : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        switch (state)
        {
            ...
            case State.Chase:
                ...
                if (DistXZ(transform.position, tgt.position) <= keepDistance + 1.5f)
                    state = State.Combat;
                break;
    ...
    void DoChase()
    {
        var tgt = sensor.CurrentTarget;
        if (!tgt) return;

        Vector3 tp = tgt.position;
        float d = DistXZ(transform.position, tp);

        if (d > keepDistance + 1.5f)
            MoveTowards(tp, chaseSpeed, rotateSpeed);
        else
    ...
    void IdleScan()
    {
        ...
        rb.MoveRotation(Quaternion.RotateTowards(
            rb.rotation,
            look,
            rotateSpeed * 0.5f * Time.fixedDeltaTime));
        AimTurretForward();
    }
    ...
    void AimTurretAt(Vector3 worldTarget)
    {
        ...
        turret.localRotation = Quaternion.RotateTowards(
            turret.localRotation, local, turretYawSpeed * Time.deltaTime);
    }
    ...
    Vector3 AvoidObstacles(Vector3 desiredDir)
    {
        ...
        Vector3 rightDir = Quaternion.AngleAxis(sideAngle, Vector3.up) * dir;
        Vector3 leftDir = Quaternion.AngleAxis(-sideAngle, Vector3.up) * dir;

        bool blockedRight = Physics.Raycast(origin, rightDir, avoidDistance, obstacleMask);
        bool blockedLeft = Physics.Raycast(origin, leftDir, avoidDistance, obstacleMask);
        ...
        if (blockedRight && !blockedLeft)
            ...
            steerDir = (dir + hit.normal).normalized;
    ...
    Vector3 WaypointPos(int i)
    {
        return waypoints[i] ? waypoints[i].position : transform.position;
    }
    ...
    static bool ArrivedXZ(Vector3 a, Vector3 b, float eps)
    {
        return DistXZ(a, b) <= eps;
    }
